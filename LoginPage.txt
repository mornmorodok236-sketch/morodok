/* ================================================
   LOGINPAGE.TSX - NEW VERSION v2.0 (2026-08-21)
   (contains useAuth + isDemoMode + signIn)
   ================================================ */
...
export const LoginPage: React.FC<LoginPageProps> = ({ setCurrentPage, setUserRole }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Real Supabase Auth — App.tsx will redirect to dashboard/admin automatically
    setBusy(true);
    setSubmittedMessage(null);
    const { error } = await signIn(email.trim().toLowerCase(), password);
    setBusy(false);
    if (error) {
      setSubmittedMessage(error === 'Invalid login credentials'
        ? 'Incorrect email or password. Please check and try again.'
        : error);
    } else {
      setSubmittedMessage('Success! Signing you in...');
    }
  };
        ...
        <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-6 sm:p-8 shadow-2xl shadow-black/80 glow-cyan">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              ...
              type="submit"
              disabled={busy}
              className="w-full py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 font-bold text-sm shadow-lg shadow-cyan-950/60 transition-all flex items-center justify-center gap-2 disabled:opacity-60"
            >
              <LogIn className="w-4 h-4" />
              <span>{busy ? 'Signing In...' : 'Log In to Client Portal'}</span>
            </button>
          </form>

          {/* Quick Demo Logins — shown only in Demo mode (Supabase not connected) */}
          {isDemoMode && (
