/* ================================================
   REGISTERPAGE.TSX - NEW VERSION v2.0 (2026-08-21)
   (contains useAuth + isDemoMode + signUp)
   ================================================ */
...
export const RegisterPage: React.FC<RegisterPageProps> = ({ setCurrentPage, setUserRole }) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (isDemoMode) {
      // Demo mode (Supabase not connected yet): behave as before
      setSuccess(true);
      setTimeout(() => {
        setUserRole('user');
    ...
    // ✅ Real Supabase Auth
    setBusy(true);
    const { error, needsEmailConfirm } = await signUp(email.trim().toLowerCase(), password, name.trim());
    setBusy(false);

    if (error) {
      setErrorMessage(error.includes('already registered')
    ...
    if (needsEmailConfirm) {
      setSuccessNote('Account created! Check your email for a confirmation link before logging in.');
    } else {
      // session is created automatically -> App.tsx will redirect to dashboard
      setSuccessNote('Account created! Signing you in...');
    }
  };
        ...
        <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-6 sm:p-8 shadow-2xl shadow-black/80 glow-cyan">
          ...
          {success ? (
            <div className="text-center py-6 space-y-4">
              ...
              <p className="text-xs text-slate-400">
                {successNote ?? 'Setting up your client portal and initializing MT4 license manager...'}
              </p>
            </div>
          ) : (
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <button
                ...
                type="submit"
                disabled={busy}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-cyan-500 via-teal-400 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 font-bold text-sm shadow-lg shadow-cyan-950/60 transition-all flex items-center justify-center gap-2 mt-2 disabled:opacity-60"
              >
                <UserPlus className="w-4 h-4" />
                <span>{busy ? 'Creating Account...' : 'Create Trader Account'}</span>
              </button>
            </form>
          )}
