/* ================================================
   USERDASHBOARD.TSX - NEW VERSION v2.0 (2026-08-21)
   (contains useAuth + licenses + claimLicenseByKey)
   ================================================ */
...
import { fetchMyLicenses, bindMt4Account, claimLicenseByKey, fetchMyOrders } from '../../lib/data';
import { 
  Key, 
  Cpu, 
  Clock, 
...
export const UserDashboard: React.FC<UserDashboardProps> = ({ setCurrentPage }) => {
  ...
  // Fetch real licenses + orders from Supabase (real mode)
  const reloadLicenses = async () => {
    if (!session?.user) return;
    const rows = await fetchMyLicenses(session.user.id);
    setLicenses(rows);
    const orders = await fetchMyOrders(session.user.id);
  ...
  // User info (real or mock)
  const displayName = isDemoMode ? MOCK_CURRENT_USER.name : (profile?.full_name ?? userEmail ?? 'Trader');
  const displayEmail = isDemoMode ? MOCK_CURRENT_USER.email : (userEmail ?? '');
  const memberSince = isDemoMode ? MOCK_CURRENT_USER.createdAt : (profile?.created_at?.split('T')[0] ?? '');
  const activePlan = isDemoMode
    ? MOCK_CURRENT_USER.activePlan
    : (licenses.find(l => l.status === 'active')?.productName ?? (licenses.length > 0 ? licenses[0].productName : 'No License Yet'));
  const initials = displayName.split(/[\s@]/).filter(Boolean).slice(0, 2).map((s) => s[0]?.toUpperCase()).join('') || 'TG';
  ...
  const handleActivateNewLicense = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    if (!isDemoMode) {
      // Real mode: claim license by key (must match your account email)
      const { error } = await claimLicenseByKey(activationInput.trim());
      if (error) {
        setActivationMsg({ text: `❌ ${error}`, success: false });
      } else {
        await reloadLicenses();
        setActivationMsg({ text: "✅ License claimed successfully! វាបង្ហាញក្នុងបញ្ជីខាងក្រោមហើយ។", success: true });
      }
      setActivationInput('');
  ...
  const handleSaveMt4Binding = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedLicenseForBind || !newMt4Account) return;

    if (!isDemoMode) {
      // Real mode: save binding to Supabase
      const { error } = await bindMt4Account(selectedLicenseForBind.id, newMt4Account.trim(), newBroker.trim() || undefined);
      if (error) {
        setActivationMsg({ text: `❌ រក្សាទុកមិនបាន: ${error}`, success: false });
        setTimeout(() => setActivationMsg(null), 5000);
        return;
      }
      await reloadLicenses();
  ...
  return (
    <div className="min-h-screen bg-[#090D16] text-slate-100 py-10 lg:py-14">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono mt-0.5">
                {displayEmail}{memberSince ? ` • Member since ${memberSince}` : ''}
              </p>
            </div>
          </div>
        ...
        {/* Order waiting for admin approval (real mode) */}
        {pendingOrders.length > 0 && (
          <div className="p-4 rounded-2xl bg-amber-950/40 border border-amber-700/60 flex items-start gap-3">
            <Clock className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
            <div className="text-sm">
              <p className="font-bold text-amber-300">
                មាន {pendingOrders.length} Order កំពុងរង់ចាំការបញ្ជាក់ការបង់ប្រាក់
              </p>
              <p className="text-amber-200/70 text-xs mt-0.5">
                {pendingOrders.map((o: any) => `${o.products?.name ?? 'Order'} (${o.plan}) — $${o.amount}`).join(' • ')}
                {' '}— License Key នឹងបង្ហាញទីនេះដោយស្វ័យប្រវត្តិ បន្ទាប់ពី Admin បញ្ជាក់រួច។
              </p>
            </div>
          </div>
        )}
