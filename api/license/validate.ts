...
export default async function handler(req: any, res: any) {
  ...
  if (!lic.mt4_account) {
    ...
    return res.status(200).json({
      ...
      message: 'This license is bound to a different MT4 account.',
    });
  } else {
    await supabase.from('licenses').update({ last_heartbeat: now.toISOString() }).eq('id', lic.id);
  }
  ...
  return res.status(200).json({
    ...
    days_left: daysLeft,
    expires_at: lic.expires_at,
    product: (lic as any).products?.name || null,
